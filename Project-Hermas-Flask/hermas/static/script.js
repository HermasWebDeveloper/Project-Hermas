...
fetch('/products')
  ...
  .then(data => {
    if (!Array.isArray(data)) {
      showToast('Failed to load products: Invalid data format.', 'error');
      return;
    }
    products = data;
    renderProducts();
...
async function downloadExcelHandler() {
    ...
    try {
        ...
        if (!response.ok) {
            let msg = 'Failed to generate Excel file';
            try {
        ...
        showToast('Failed to download Excel file: ' + error, 'error');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Prevent errors if productGroupsContainerDOM is missing
    if (!productGroupsContainerDOM) {
        showToast('Critical error: Product groups container not found in HTML.', 'error', 10000);
        return;
    }
    ...
    if (clearBtn) {
        clearBtn.addEventListener('click', function() {
            ...
            // Only clear this app's localStorage keys
            localStorage.removeItem(STORAGE_KEY_CART);
            localStorage.removeItem(STORAGE_KEY_CUSTOMER_INFO);

            // Optionally clear sessionStorage keys if you use them (none in this code)
